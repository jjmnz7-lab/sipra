-- Migración: 011_hot_fixes
-- Descripción: Correcciones de seguridad y performance (Fase 1)

-- 1. Arreglar NULL silencioso en procesar_recargos_v1 (TD-10)
...
-- 2. Índice faltante en movimiento (TD-14)
CREATE INDEX IF NOT EXISTS idx_movimiento_academia_estado_fecha 
ON public.movimiento (academia_id, estado, fecha_pago)
WHERE estado = 'registrado';

-- 3. Índice funcional en cargo para anti-duplicados (TD-03)
CREATE INDEX IF NOT EXISTS idx_cargo_concepto_slug 
ON public.cargo (academia_id, persona_id, lower(trim(concepto)), fecha_vencimiento)
WHERE estado_financiero != 'anulado';
